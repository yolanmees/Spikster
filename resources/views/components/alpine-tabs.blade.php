...
<div class="pb-5">
    ...
    <div class="sm:hidden">
        ...
        <select x-model="{{ $model }}"
            class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white focus:border-blue-500 focus:ring-blue-500 text-sm">
            @foreach($tabs as $tab)
        ...
        <div class="border-b-2 border-gray-200 dark:border-gray-700">
            <nav class="flex -mb-px space-x-1" aria-label="Tabs">
                @foreach($tabs as $tab)
                    ...
                    >
                        @if(!empty($tab['icon']))
                            <span :class="{{ $model }} === '{{ $tab['key'] }}' ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-500'"
