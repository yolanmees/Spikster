...
@section('title')
    Edit DNS Record - {{ $domain->domain }}
@endsection

@section('content')
    <div class="max-w-2xl mx-auto space-y-6">
        <x-page-header title="Edit DNS Record" subtitle="Update DNS record for {{ $domain->domain }}" size="section">
            <x-slot name="actions">
        ...
        <div class="rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <form method="POST" action="{{ route('domain.dns.update', [$domain->domain_id, $dnsRecord->id]) }}" class="space-y-6 p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid gap-5 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        ...
                        <select id="type" name="type" required
                            ...
                            <option value="A" {{ old('type', $dnsRecord->type) == 'A' ? 'selected' : '' }}>A - IPv4 Address</option>
                            <option value="AAAA" {{ old('type', $dnsRecord->type) == 'AAAA' ? 'selected' : '' }}>AAAA - IPv6 Address</option>
                            <option value="CNAME" {{ old('type', $dnsRecord->type) == 'CNAME' ? 'selected' : '' }}>CNAME - Canonical Name</option>
                            <option value="MX" {{ old('type', $dnsRecord->type) == 'MX' ? 'selected' : '' }}>MX - Mail Exchange</option>
                            <option value="TXT" {{ old('type', $dnsRecord->type) == 'TXT' ? 'selected' : '' }}>TXT - Text Record</option>
                            <option value="NS" {{ old('type', $dnsRecord->type) == 'NS' ? 'selected' : '' }}>NS - Name Server</option>
                            <option value="SRV" {{ old('type', $dnsRecord->type) == 'SRV' ? 'selected' : '' }}>SRV - Service Record</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="ttl" id="ttl" value="{{ old('ttl', $dnsRecord->ttl) }}" min="60" max="86400"
                            class="mt-1.5 block w-full rounded-lg border border-zinc-200 bg-white px-3 py-2.5 font-mono text-sm outline-none focus:border-purple-700 focus:ring-2 focus:ring-purple-700/20 dark:border-zinc-800 dark:bg-zinc-950 dark:text-white">
                        <p class="mt-1 text-xs text-zinc-400">Time to live in seconds (default: 3600 = 1 hour)</p>
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="number" name="priority" id="priority" value="{{ old('priority', $dnsRecord->priority) }}" min="0" max="65535"
                            class="mt-1.5 block w-full rounded-lg border border-zinc-200 bg-white px-3 py-2.5 font-mono text-sm outline-none focus:border-purple-700 focus:ring-2 focus:ring-purple-700/20 dark:border-zinc-800 dark:bg-zinc-950 dark:text-white">
