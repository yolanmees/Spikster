    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Update DNS record for {{ $domain->domain }}
            </p>
        </div>

        <!-- Form -->
        <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg">
            <form method="POST" action="{{ route('domain.dns.update', [$domain->domain_id, $dnsRecord->id]) }}"
                class="space-y-6 p-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select id="type" name="type" required
                        ...
                        <option value="A" {{ old('type', $dnsRecord->type) == 'A' ? 'selected' : '' }}>A - IPv4 Address
                        </option>
                        <option value="AAAA" {{ old('type', $dnsRecord->type) == 'AAAA' ? 'selected' : '' }}>AAAA - IPv6
                            Address</option>
