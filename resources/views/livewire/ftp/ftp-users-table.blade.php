<div class="space-y-6">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div>
                                        ...
                                        @if ($user->notes)
                                            <div class="text-sm text-gray-500">{{ Str::limit($user->notes, 30) }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">{{ Str::limit($user->home_directory, 40) }}</div>
                                <div class="text-xs text-gray-500">{{ $user->max_connections }} connections •
                                    @if ($user->require_ssl)
                                    <span class="text-green-600">SSL</span>@else<span class="text-red-600">No
                                            SSL</span>
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $user->getFormattedQuota() }}</div>
                                <div class="text-xs text-gray-500">{{ $user->getFormattedUsage() }} used</div>
                                @if ($user->quota_mb > 0)
                                    <div class="mt-1 w-full bg-gray-200 rounded-full h-1.5">
                                        <div class="bg-{{ $user->getQuotaBadgeColor() }}-600 h-1.5 rounded-full"
                                            style="width: {{ min($user->getQuotaUsagePercentage(), 100) }}%"></div>
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-{{ $user->getStatusBadgeColor() }}-100 text-{{ $user->getStatusBadgeColor() }}-800">
                                    {{ $user->getStatusText() }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $user->getLastLoginHuman() }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    ...
                                    {{-- Toggle Status --}}
                                    @if ($user->is_active)
                                        <button wire:click="toggleStatus('{{ $user->ftp_user_id }}')"
                                            wire:confirm="Are you sure you want to disable this user?" title="Disable"
                                    ...
                                    @else
                                        <button wire:click="toggleStatus('{{ $user->ftp_user_id }}')" title="Enable"
                                    ...
                                    {{-- Unlock if locked --}}
                                    @if ($user->isLocked())
                                        <button wire:click="unlockUser('{{ $user->ftp_user_id }}')" title="Unlock"
    ...
    {{-- Modals --}}
    @include('livewire.ftp.partials.create-modal')
    @include('livewire.ftp.partials.edit-modal')
    @include('livewire.ftp.partials.delete-modal')
    @include('livewire.ftp.partials.password-modal')
    @include('livewire.ftp.partials.quota-modal')
    @include('livewire.ftp.partials.connection-info-modal')
</div>
